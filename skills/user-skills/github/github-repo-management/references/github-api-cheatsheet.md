...
## Push Without gh CLI — Complete Workflow

When `gh` is not available (e.g., Raspberry Pi, headless server), here is the full create-remote-repo-and-push workflow:

### 1. Create the private repo via API
...
curl -s -X POST \
  ...
  -d "{\"name\": \"$REPO_NAME\", \"description\": \"$DESC\", \"private\": true, \"auto_init\": false}"
```

### 2. Configure credentials (do NOT embed token in remote URL)

```bash
# Safe: credential.helper + credentials file
...
> ⚠️ **Do NOT** use `git remote add origin https://USERNAME:TOKEN@...` — it fails with special characters and GitHub explicitly rejects the `USER:TOKEN@` format.

See also: `github-auth` skill, [references/token-handling.md](token-handling.md)
