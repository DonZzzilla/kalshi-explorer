...
## "Cannot add message" — IMAP backend required for send

`template send` and `message send` both save a copy to Sent via IMAP after SMTP
delivery. If the account config only has `message.send.backend.*` (SMTP) and is
**missing** the top-level `backend.*` (IMAP) block, you get:
...
The fix is to add a `backend.*` block directly under `[accounts.NAME]` — see
the Gmail example above for the exact keys. This is separate from
`message.send.backend.*` (which is SMTP-only).
