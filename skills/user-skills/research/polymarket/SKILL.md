...
For the Kalshi API (a CFTC-regulated prediction markets exchange with similar concepts), see `references/kalshi-api.md`. Key differences from Polymarket:
- Auth uses API key + HMAC-SHA256 (no wallet needed for trading)
- Prices in cents (0–100) instead of dollars (0–1)
- Same scanner workflow applies
