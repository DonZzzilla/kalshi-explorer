...
**Full guide:** `references/ascii-video.md` (ex-`ascii-video` skill)

For animated ASCII output (MP4, GIF), use a single Python script with NumPy + Pillow + ffmpeg.
...
| Audio-reactive | Audio file | Generatives visuals driven by audio features |
...
**Critical:** Use `tonemap()` for brightness (percentile-based), never `canvas * N` multipliers — they clip highlights.

**Stack:** Python 3.10+, NumPy, SciPy (FFT), Pillow, ffmpeg, concurrent.futures. No GPU required.

> For full creative direction, scene functions, effects catalog, shader pipeline, and troubleshooting, load `references/ascii-video.md`.
