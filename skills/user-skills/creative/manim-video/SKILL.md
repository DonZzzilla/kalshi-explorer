...
The `hyperframes` pip package (`pip install hyperframes`) is a **data science library** for labeled numpy arrays — it has NO video rendering capability and no CLI entry point. Do NOT install it for video work. If a user mentions "hyperframes render", redirect them to Manim or another actual video tool.
